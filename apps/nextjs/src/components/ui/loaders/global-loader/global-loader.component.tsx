...
import { setGlobalLoaderState } from '@/redux/common/common.reducer'
...
const GlobalLoader: FC<GlobalLoaderProps> = ({ initTime = 0 }): null | JSX.Element => {
  const dispatch: Dispatch<AnyAction> = useDispatch()
  const globalLoaderActive: boolean = useSelector(selectGlobalLoaderActive)

  useEffect((): void => {
    setTimeout(() => dispatch(setGlobalLoaderState({ state: false })), initTime)
  }, [dispatch, initTime])

  return (
    <>
      {globalLoaderActive ? (
        <div className="z-100 absolute top-0 flex h-screen w-full items-center justify-center bg-slate-200">
          <svg
            className="w-3/12 fill-primary md:w-1/12"
